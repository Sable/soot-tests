...
TARGET_PROGRAM="./target/lucene-all-6.0.0-SNAPSHOT" # path to the target jar / apk Soot will read
...
java -Xmx4096m -cp $CLASSPATH soot.Main -allow-phantom-refs $SOOT_OPTIONS -process-dir $SOOT_INPUT -d $SOOT_OUTPUT_DIR -validate
R=$?
echo "soot: "$R
rm -rf "test-reports."${TEST_TARGET}
rm -rf "tests."${TEST_TARGET}
ant -Dlucene=$TEST_TARGET test
R=$(($? + $R))
echo "ant: "$R
return $R
}
