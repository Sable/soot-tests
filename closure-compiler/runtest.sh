...
TARGET_PROGRAM="./target/compiler" # path to the target jar / apk Soot will read
...
java -jar $SOOT_JAR -keep-line-number -allow-phantom-refs $SOOT_OPTIONS -process-dir $SOOT_INPUT -d $SOOT_OUTPUT_DIR
R=$?
echo "soot: "$R
rm -rf "test-reports."${TEST_TARGET}
rm -rf "tests."${TEST_TARGET}
ant -Dtargetdir=$TEST_TARGET test
R=$(($? + $R))
echo "ant: "$R
return $R
}
