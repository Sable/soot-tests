...
TARGET_PROGRAM="./target/dacapo-2006-10-MR2" # path to the target jar / apk Soot will read
...
java -cp $CLASSPATH -Xmx4096m soot.Main -allow-phantom-refs $SOOT_OPTIONS -process-dir $SOOT_INPUT -d $SOOT_OUTPUT_DIR -validate
R=$?
echo "soot: "$R
rm -rf "test-reports."${TEST_TARGET}
rm -rf "tests."${TEST_TARGET}
java -jar ${TARGET_PROGRAM}.jar -s small antlr bloat chart eclipse fop hsqldb jython luindex lusearch pmd xalan
R=$(($? + $R))
echo "ant: "$R
return $R
}
